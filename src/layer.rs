...
/// A single bar layer. Today this is always a button row; later phases add a
/// `Slider` variant. The wrapping enum lets the event loop stay layer-kind
/// agnostic and keeps the door open for non-button layers.
pub enum Layer {
